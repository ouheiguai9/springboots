...
public interface TriColorLedLogRepository extends CrudRepository<TriColorLedLog, String> {
    Page<TriColorLedLog> findAllByDevice_IdAndTimeBetweenAndStatusAndDurationGreaterThan(Pageable pageable, String deviceId, LocalDateTime start, LocalDateTime end, TriColorLedLog.Status status, Long duration);

    List<TriColorLedLog> findAllByDevice_IdAndTimeBetweenOrderByTimeAsc(String deviceId, LocalDateTime start, LocalDateTime end);

    Optional<TriColorLedLog> findFirstByDevice_IdAndTimeIsLessThanOrderByTimeDesc(String deviceId, LocalDateTime end);
    ...
    @Query("select new TriColorLedLog(log.device, log.status, count(log.duration)) from TriColorLedLog log where log.device in ?1 and log.status = ?2 and log.time between ?3 and ?4 and log.duration > ?5 group by log.status, log.device")
